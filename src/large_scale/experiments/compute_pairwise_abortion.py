"""
Compute O(n^2) pairwise comparisons for the abortion topic.

This script:
- Loads 900 statements for "What should guide laws concerning abortion?"
- Samples 5 discriminative personas (seed=42)
- Generates all ordered pairs (i,j) where i != j
- Batches 10 comparisons per API call
- Uses 100 parallel workers with exponential backoff
- Writes results immediately to CSV (one per persona)
- Total: 5 personas × 809,100 comparisons = 4,045,500 comparisons
"""
...
# File locks for concurrent CSV writing (one per persona)
file_locks = {}
...
def setup_logging():
    ...
    log_file = log_dir / "pairwise_abortion.log"
    logging.basicConfig(
        level=logging.INFO,
...
def load_statements() -> List[str]:
    """Load statements for abortion topic."""
...
def load_and_sample_personas(seed: int = 42, num_personas: int = 5) -> List[Dict]:
...
def compare_batch_api(persona_desc: str, statements: List[str], batch: List[Tuple[int, int]]) -> List[Dict]:
    ...
    Returns list of preferences: [{"pair_idx": 0, "preference": "A"}, ...]
    """
    # Build prompt
    comparisons_text = []
...
def write_results_to_csv(persona_idx: int, results: List[Tuple[int, int, int]]):
    """
    ...
    output_file = output_dir / f"pairwise_abortion_persona_{persona_idx}.csv"
...
def process_batch(persona_idx: int, persona_desc: str, statements: List[str], batch: List[Tuple[int, int]]) -> int:
    ...
    try:
        ...
        write_results_to_csv(persona_idx, csv_rows)
...
def get_completed_pairs(persona_idx: int) -> set:
    ...
    output_file = Path(f"/Users/jeqcho/single-winner-generative-social-choice/data/large_scale/prod/pairwise_abortion_persona_{persona_idx}.csv")
    ...
    logging.info(f"Persona {persona_idx}: Found {len(completed)} completed pairs")
    return completed

def compute_pairwise_comparisons(num_personas: int = 5, seed: int = 42, max_workers: int = 100):
    """
    Main function to compute all pairwise comparisons.
    
    Args:
        num_personas: Number of personas to sample
        seed: Random seed for persona sampling
        max_workers: Number of parallel workers
    """
    log_dir = setup_logging()
    logging.info("=" * 80)
    logging.info("STARTING O(n^2) PAIRWISE COMPARISON COMPUTATION")
    logging.info("=" * 80)
    ...
    logging.info(f"Batches of 10: {total_comparisons // 10:,}")
    ...
    for persona_idx, persona_desc in sampled_personas:
        # Get already completed pairs for this persona
        completed_pairs = get_completed_pairs(persona_idx)
        
        # Filter out completed pairs
        remaining_pairs = [pair for pair in all_pairs if pair not in completed_pairs]
        
        if not remaining_pairs:
            logging.info(f"Persona {persona_idx}: All comparisons already completed")
            continue
        
        logging.info(f"Persona {persona_idx}: {len(remaining_pairs):,} pairs remaining")
        
        # Batch the remaining pairs
        for batch in batch_pairs(remaining_pairs, batch_size=10):
    ...
    if total_batches == 0:
        logging.info("No work to do - all comparisons already completed!")
        return
    ...
    logging.info("=" * 80)
    logging.info(f"COMPLETED: {comparisons_completed:,} comparisons processed")
    logging.info(f"Logs saved to: {log_dir}")
    logging.info("=" * 80)

if __name__ == "__main__":
    compute_pairwise_comparisons(num_personas=5, seed=42, max_workers=100)
