...
100 statements, 3 personas (unique pairs only: i < j)

This script:
- Loads first 100 statements for "What should guide laws concerning abortion?"
- Samples 3 discriminative personas (seed=42)
- Generates unique pairs (i,j) where i < j only
- Batches 10 comparisons per API call
- Uses 100 parallel workers with exponential backoff
- Total: 3 personas × 4,950 comparisons = 14,850 comparisons
"""
...
def generate_unique_pairs(n: int) -> List[Tuple[int, int]]:
    """Generate unique pairs (i, j) where i < j for n items."""
    pairs = [(i, j) for i in range(n) for j in range(i+1, n)]
    logging.info(f"Generated {len(pairs)} unique pairs for {n} statements (i < j)")
    return pairs
...
def compute_pairwise_comparisons(num_statements: int = 100, num_personas: int = 3, seed: int = 42, max_workers: int = 100):
    """
    Main function to compute all pairwise comparisons.
    
    Args:
        num_statements: Number of statements to use
    ...
    logging.info("=" * 80)
    logging.info("STARTING PAIRWISE COMPARISON: 100 STATEMENTS, 3 PERSONAS")
    logging.info("=" * 80)
    ...
    # Generate unique pairs (i < j)
    all_pairs = generate_unique_pairs(len(statements))
    ...
    logging.info(f"Output files: data/large_scale/prod/pairwise_100_abortion_persona_*.csv")
    logging.info(f"Logs saved to: {log_dir}")
    logging.info("=" * 80)

if __name__ == "__main__":
    compute_pairwise_comparisons(num_statements=100, num_personas=3, seed=42, max_workers=100)
