"""
Test script for the chunked Borda insertion algorithm.

This script validates the chunked insertion approach by:
1. Taking known alternatives from existing preference rankings
2. Removing them from the rankings
3. Using chunked Borda to predict their position
...
Usage:
    # Run test for both topics
    uv run python -m src.experiment_utils.test_chunked_insertion --topic abortion
    uv run python -m src.experiment_utils.test_chunked_insertion --topic environment
    
    # Run both topics
    uv run python -m src.experiment_utils.test_chunked_insertion --all-topics
"""
...
from .chunked_insertion import insert_statement_chunked_borda, get_chunk_positions_detail
...
def test_chunked_insertion_for_voter(
    voter_idx: int,
    persona: str,
    ...
    """
    Test chunked insertion for a single voter and alternative.
    ...
    # Get detailed chunk positions
    try:
        detail = get_chunk_positions_detail(
            persona=persona,
        ...
        chunk_positions = detail["chunk_positions"]
...
def run_test_for_topic(
    ...
    """
    Run chunked insertion test for a topic.
    ...
    def process_task(task):
        return test_chunked_insertion_for_voter(
...
def main():
    parser = argparse.ArgumentParser(description="Test chunked Borda insertion algorithm")
