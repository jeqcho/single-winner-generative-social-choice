"""
Analyze and visualize original insertion test results.
...
Usage:
    uv run python -m src.experiment_utils.analyze_original_insertion
"""
...
def plot_position_error_distribution(topics: List[str]) -> None:
    ...
    ax.set_title('Position Error Distribution (Original Insertion)\n(Negative = predicted too preferred)', fontsize=14)
...
def plot_predicted_vs_original_scatter(topics: List[str]) -> None:
    ...
    ax.set_title('Predicted vs Original Position (Original Insertion)\n(Points below line = predicted too preferred)', fontsize=14)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
...
def main():
    ...
    # Print summary
    print("\n" + "="*60)
    print("ORIGINAL INSERTION TEST RESULTS")
    print("="*60)
