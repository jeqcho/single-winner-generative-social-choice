"""
Test script for the pairwise Borda insertion algorithm.

This script validates the pairwise insertion approach by:
1. Taking known alternatives from existing preference rankings
2. Removing them from the rankings
3. Using pairwise Borda to predict their position
...
- borda_score (win rate against existing statements)

Usage:
    # Run test for a single topic
    uv run python -m src.experiment_utils.test_pairwise_insertion --topic abortion
    uv run python -m src.experiment_utils.test_pairwise_insertion --topic environment
    
    # Run both topics
    uv run python -m src.experiment_utils.test_pairwise_insertion --all-topics
"""
...
from .pairwise_insertion import insert_statement_pairwise_borda, get_pairwise_detail
...
def load_preferences_for_rep(topic_short, rep_id):
...
def load_epsilons_for_rep(topic_short, rep_id):
...
def select_test_alternatives(n_alts, n_test=10, seed=42):
    """
...
def get_original_position_for_voter(preferences, voter_idx, alt_idx):
    """
...
def remove_alt_from_ranking(ranking, alt_to_remove):
    """Remove an alternative from a ranking."""
    return [alt for alt in ranking if alt != alt_to_remove]


def test_pairwise_insertion_for_voter(
    voter_idx,
    persona,
    original_ranking,
    statements,
    ...
    """
    Test pairwise insertion for a single voter and alternative.
    ...
    # Get detailed pairwise results
    try:
        detail = get_pairwise_detail(
            persona=persona,
            ...
            method="pairwise_insertion_test",
            rep=rep_id,
            voter_idx=voter_idx,
        )
        
        predicted_position = detail["final_position"]
        borda_score = detail["borda_score"]
        wins = detail["wins"]
        ...
        return {
            ...
            "wins": wins,
            "total_comparisons": detail["total_comparisons"],
            "success": True,
        }
...
def run_test_for_topic(
    ...
    """
    Run pairwise insertion test for a topic.
    """
    ...
    # Select test alternatives (same seed as other tests for comparability)
    ...
    logger.info(f"Total tasks: {len(tasks)} ({n_test_alts} alts x {n_voters} voters)")
    logger.info(f"Each task requires {n_alts - 1} pairwise comparisons")
    logger.info(f"Total API calls expected: {len(tasks) * (n_alts - 1)}")
    
    # Run tests in parallel (per-voter parallelization)
    start_time = time.time()
    
    def process_task(task):
        return test_pairwise_insertion_for_voter(
    ...
    if successful_results:
        ...
        borda_scores = [r["borda_score"] for r in successful_results]
        ...
        summary = {
            ...
            "borda_score_mean": np.mean(borda_scores),
            "borda_score_std": np.std(borda_scores),
        ...
        for test_alt_idx in test_alt_indices:
            ...
            if alt_results:
                ...
                alt_summaries[str(test_alt_idx)] = {
                    ...
                    "mean_borda_score": np.mean([r["borda_score"] for r in alt_results]),
...
def save_results(results, topic_short, rep_id):
...
def main():
    parser = argparse.ArgumentParser(description="Test pairwise Borda insertion algorithm")
    ...
    for topic_short in topics_to_test:
        ...
        if summary.get("successful_tests", 0) > 0:
            ...
            logger.info(f"  Borda score: mean={summary['borda_score_mean']:.3f}")
