#!/usr/bin/env python3
"""
Fix GPT** epsilon values for electoral topic.
...
The new_statement text is valid; only the epsilon needs recomputation.

Usage:
    uv run python scripts/fix_electoral_double_star.py
"""
...
# Setup logging
LOG_FILE = Path(__file__).parent.parent / "logs" / f"fix_electoral_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Topic to fix
TOPIC_SHORT = "electoral"
TOPIC_SLUG = "what-reforms-if-any-should-replace-or-modify-the-e"
...
def main():
    """Fix GPT** epsilon values for all electoral reps."""
    logger.info("="*60)
    logger.info("Fixing GPT** epsilon values for electoral topic")
    logger.info("="*60)
    ...
    # Find all electoral rep directories
    electoral_dir = PHASE2_DATA_DIR / TOPIC_SHORT
    if not electoral_dir.exists():
        logger.error(f"Electoral directory not found: {electoral_dir}")
        sys.exit(1)
    
    pref_files = list(electoral_dir.glob("**/preferences.json"))
    logger.info(f"Found {len(pref_files)} electoral reps to fix")
