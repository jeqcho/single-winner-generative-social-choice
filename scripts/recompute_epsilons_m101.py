#!/usr/bin/env python3
"""
Recompute epsilons for methods with insertion_positions using m=101.

This script updates epsilon values for methods that generate new statements
(chatgpt_double_star, chatgpt_triple_star, random_insertion, etc.) by:
1. Constructing a 101-alternative preference profile
2. Computing epsilon with m=101 (natural, no override)
3. Updating the results.json files in place

Scope: uniform voters, persona_no_context only

Usage:
    uv run python scripts/recompute_epsilons_m101.py
    uv run python scripts/recompute_epsilons_m101.py --dry-run  # Preview changes
"""
...
def process_results_file(
    ...
    """
    Process a results.json file and recompute epsilons for methods with insertion_positions.
    
    Args:
        results_path: Path to results.json
        preferences: Original 100x100 preference matrix
        dry_run: If True, don't write changes
        
    Returns:
        Dict mapping method -> {old_epsilon, new_epsilon}
    """
    changes = {}
    ...
    for method in METHODS_WITH_INSERTION:
        ...
        # Check if method has insertion_positions
        ...
        # Handle None values in insertion_positions
        ...
        # Compute new epsilon with m=101
...
def main():
    """Recompute epsilons for all uniform voter data."""
    parser = argparse.ArgumentParser(description="Recompute epsilons with m=101")
    ...
    for topic in tqdm(TOPICS, desc="Processing topics"):
        topic_dir = DATA_DIR / topic / "uniform" / ALT_DIST
            ...
            for mini_rep_idx in range(N_MINI_REPS):
                ...
                if changes:
                    ...
                    for method, change in changes.items():
                        ...
                        logger.debug(
                            f"{topic}/rep{rep_idx}/mini_rep{mini_rep_idx} - {method}: "
                            f"{change['old_epsilon']:.4f} -> {change['new_epsilon']:.4f} "
                            f"(diff: {change['diff']:.4f})" if change['diff'] is not None else ""
                        )
