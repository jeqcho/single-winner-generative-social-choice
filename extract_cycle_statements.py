...
def main():
    ...
    # Define cycles found
    cycles = {
        'pairwise_100_abortion_persona_1.csv': [13, 7, 90],
        'pairwise_100_abortion_persona_40.csv': [0, 51, 12],
        'pairwise_100_abortion_persona_7.csv': [0, 1, 22]
    }

    # Extract statements for each cycle
    results = {}
    for filename, cycle_ids in cycles.items():
        print(f"\nExtracting statements for {filename}")
        results[filename] = extract_cycle_statements(statements, cycle_ids)
    ...
    with open(output_path, 'w') as f:
        ...
        for filename, cycle_data in results.items():
            f.write(f"## {filename}\n\n")
